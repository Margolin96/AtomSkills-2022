...
const mountVisData = (vm, propName /* , DataSet, DataView */) => {
  ...
  if (!(vm[propName] instanceof DataSet || vm[propName] instanceof DataView)) {
    ...
    // Rethrow all events
    data.on('*', (event, properties, senderId) => vm.$emit(`${propName}-${event}`, { event, properties, senderId }));
...
const translateEvent = (event) => event.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase();
